...
public class InternCoinTableActivity extends BaseCoinTableActivity {
    ...
    protected void initTableLogic() {
        ...
        // FIX: Find the button, MAKE IT VISIBLE, then set listener
        ...
        // FIX: Find the button, MAKE IT VISIBLE, then set listener
        View btnSpecial = findViewById(R.id.openAddingYearSpecialII);
        if (btnSpecial != null) {
        ...
        table = new TableItem[MAX_ROWS][9];
        // Handle potential null coinCountry
        TableItem headerCountry = (coinCountry != null) ? coinCountry : X;
        table[0] = new TableItem[]{headerCountry, ONE, TWO, FIVE, TEN, TWENTY, FIFTY, I, II};
    ...
    protected String getTextForCell(int row, int col) {
        if (row == 0 && col > 0) return findValueString(table[row][col]); // "ONE" -> "1"
        if (row == 0 && col == 0) return coinCountry.toString(); // "FRANCE"

        // Ensure years are strings
        if (col == 0 && row > 0) {
            // Because we populate the table with X (Placeholder) initially,
            // we need to look at our tableYears array or the modified table value.
            // Since optimizeTableLayout no longer shifts data, we must update the year text
            // inside processData directly into the table array or handle it here.
            // SEE FIX BELOW IN processData
            if (table[row][0] != null && table[row][0] != X) {
                return table[row][0].toString();
            }
        }
    ...
    private void processData(List<Coin> coins) {
        ...
        for (int year : coinYears) {
            ...
            // Fix: Store the year directly in the table array so getTextForCell picks it up
            // You might need a way to store "2002" in TableItem.
            // If TableItem is an Enum, we can't store arbitrary numbers.
            // SOLUTION: We update the TextView directly here.
            tableYears[pointer] = year < 10 ? "0" + year : String.valueOf(year);
            if(cellViews[pointer+1][0] instanceof TextView) {
                ((TextView) cellViews[pointer+1][0]).setText(tableYears[pointer]);
            }

            for (Coin coin : coinMap.get(year)) {
                int colIndex = getColumnForValue(coin.getValue());
                if (colIndex != -1) table[pointer + 1][colIndex] = MISSING;
            }
            pointer++;
        }
    ...
    protected void setupAdminClickListeners() {
        for (int i = 1; i < table.length; i++) {
            for (int j = 1; j < table[1].length; j++) {
                if(cellViews[i][j] != null) cellViews[i][j].setOnClickListener(this::click);
            }
        }
    }
